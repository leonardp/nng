...
static void
nni_zephyr_udp_doerror(nni_plat_udp *udp, int rv)
{
	nni_aio *aio;
...
static void
nni_zephyr_udp_doclose(nni_plat_udp *udp)
{
	nni_zephyr_udp_doerror(udp, NNG_ECLOSED);
}

static void
nni_zephyr_udp_dorecv(nni_plat_udp *udp)
{
	nni_aio * aio;
...
static void
nni_zephyr_udp_dosend(nni_plat_udp *udp)
{
	nni_aio * aio;
	...
	while ((aio = nni_list_first(q)) != NULL) {
		...
		len = nni_zephyr_nn2sockaddr(&ss, nni_aio_get_input(aio, 0));
		if (len < 1) {
			rv = NNG_EADDRINVAL;
		} else {
			unsigned     niov;
...
static void
nni_zephyr_udp_cb(nni_zephyr_pfd *pfd, unsigned events, void *arg)
{
	...
	if (events &
		...
		if (events) {
			int rv;
			rv = nni_zephyr_pfd_arm(udp->udp_pfd, events);
			if (rv != 0) {
				nni_zephyr_udp_doerror(udp, rv);
			}
		}
	}
...
nni_plat_udp_open(nni_plat_udp **upp, nni_sockaddr *bindaddr)
{
	...
	if ((salen = nni_zephyr_nn2sockaddr(&sa, bindaddr)) < 1) {
	...
	if ((rv = nni_zephyr_pfd_init(&udp->udp_pfd, udp->udp_fd)) != 0) {
		(void) close(udp->udp_fd);
		nni_mtx_fini(&udp->udp_mtx);
		NNI_FREE_STRUCT(udp);
		return (rv);
	}
	nni_zephyr_pfd_set_cb(udp->udp_pfd, nni_zephyr_udp_cb, udp);
...
{
	nni_zephyr_pfd_fini(udp->udp_pfd);
...
nni_plat_udp_recv(nni_plat_udp *udp, nni_aio *aio)
{
	...
	if (nni_list_first(&udp->udp_recvq) == aio) {
		if ((rv = nni_zephyr_pfd_arm(udp->udp_pfd, POLLIN)) != 0) {
			nni_aio_list_remove(aio);
			nni_aio_finish_error(aio, rv);
		}
	}
...
nni_plat_udp_send(nni_plat_udp *udp, nni_aio *aio)
{
	...
	if (nni_list_first(&udp->udp_sendq) == aio) {
		if ((rv = nni_zephyr_pfd_arm(udp->udp_pfd, POLLOUT)) != 0) {
			nni_aio_list_remove(aio);
			nni_aio_finish_error(aio, rv);
		}
	}
...
nni_plat_udp_sockname(nni_plat_udp *udp, nni_sockaddr *sa)
{
	...
	return (nni_zephyr_sockaddr2nn(sa, &ss, sz));
}
