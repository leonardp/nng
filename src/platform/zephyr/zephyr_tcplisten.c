...
tcp_listener_doaccept(nni_tcp_listener *l)
{
	...
	while ((aio = nni_list_first(&l->acceptq)) != NULL) {
...
#endif
#endif
				rv = nni_zephyr_pfd_arm(l->pfd, NNI_POLL_IN);
				if (rv != 0) {
					nni_aio_list_remove(aio);
		...
		if ((rv = nni_zephyr_tcp_alloc(&c, NULL)) != 0) {
			close(newfd);
			nni_aio_list_remove(aio);
			nni_aio_finish_error(aio, rv);
			continue;
		...
		if ((rv = nni_zephyr_pfd_init(&pfd, newfd)) != 0) {
			close(newfd);
...
static void
tcp_listener_cb(nni_zephyr_pfd *pfd, unsigned events, void *arg)
{
...
// traditional SO_REUSEADDR.
	...
	nni_zephyr_pfd_set_cb(pfd, tcp_listener_cb, l);
...
tcp_listener_get_locaddr(void *arg, void *buf, size_t *szp, nni_type t)
{
	...
	if (l->started) {
		...
		(void) getsockname(
		    nni_zephyr_pfd_fd(l->pfd), (void *) &ss, &len);
		(void) nni_zephyr_sockaddr2nn(&sa, &ss, len);
	} else {
