...
ipc_dialer_free(void *arg)
{
	...
	nni_zephyr_ipc_dialer_rele(d);
}

void
nni_zephyr_ipc_dialer_rele(ipc_dialer *d)
{
...
static void
ipc_dialer_cb(nni_zephyr_pfd *pfd, unsigned ev, void *arg)
{
	nni_ipc_conn *  c = arg;
...
ipc_dialer_dial(void *arg, nni_aio *aio)
{
	...
	if (((len = nni_zephyr_nn2sockaddr(&ss, &d->sa)) == 0) ||
	...
	if ((rv = nni_zephyr_ipc_alloc(&c, &d->sa, d)) != 0) {
		(void) close(fd);
		nni_zephyr_ipc_dialer_rele(d);
		nni_aio_finish_error(aio, rv);
		return;
	...
	nni_zephyr_pfd_set_cb(pfd, ipc_dialer_cb, c);
