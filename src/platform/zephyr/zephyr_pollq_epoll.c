...
typedef struct nni_zephyr_pollq nni_zephyr_pollq;
...
// nni_zephyr_pollq is a work structure that manages state for the epoll-based
// pollq implementation
struct nni_zephyr_pollq {
	nni_mtx  mtx;
...
struct nni_zephyr_pfd {
	nni_list_node    node;
	nni_zephyr_pollq *pq;
	int              fd;
	nni_zephyr_pfd_cb cb;
	void *           arg;
	bool             closed;
	bool             closing;
	bool             reap;
...
static nni_zephyr_pollq nni_zephyr_global_pollq;

int
nni_zephyr_pfd_init(nni_zephyr_pfd **pfdp, int fd)
{
	nni_zephyr_pfd *    pfd;
	nni_zephyr_pollq *  pq;
...
int
nni_zephyr_pfd_arm(nni_zephyr_pfd *pfd, unsigned events)
{
	nni_zephyr_pollq *pq = pfd->pq;
...
int
nni_zephyr_pfd_fd(nni_zephyr_pfd *pfd)
{
	return (pfd->fd);
}

void
nni_zephyr_pfd_set_cb(nni_zephyr_pfd *pfd, nni_zephyr_pfd_cb cb, void *arg)
{
	nni_mtx_lock(&pfd->mtx);
	pfd->cb  = cb;
	pfd->arg = arg;
...
void
nni_zephyr_pfd_close(nni_zephyr_pfd *pfd)
{
	nni_mtx_lock(&pfd->mtx);
	if (!pfd->closing) {
		nni_zephyr_pollq *  pq = pfd->pq;
...
void
nni_zephyr_pfd_fini(nni_zephyr_pfd *pfd)
{
	nni_zephyr_pollq *pq = pfd->pq;

	nni_zephyr_pfd_close(pfd);
...
static void
nni_zephyr_pollq_reap(nni_zephyr_pollq *pq)
{
	nni_zephyr_pfd *pfd;
	nni_mtx_lock(&pq->mtx);
...
static void
nni_zephyr_poll_thr(void *arg)
{
	nni_zephyr_pollq *  pq = arg;
	...
	for (;;) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if ((ev->data.ptr == NULL) &&
				...
				nni_zephyr_pfd *  pfd = ev->data.ptr;
				nni_zephyr_pfd_cb cb;
				void *           cbarg;
				unsigned         mask;
...
static void
nni_zephyr_pollq_destroy(nni_zephyr_pollq *pq)
{
	uint64_t one = 1;
...
static int
nni_zephyr_pollq_add_eventfd(nni_zephyr_pollq *pq)
{
...
static int
nni_zephyr_pollq_create(nni_zephyr_pollq *pq)
{
	int rv;
	...
	NNI_LIST_INIT(&pq->reapq, nni_zephyr_pfd, node);
	nni_mtx_init(&pq->mtx);

	if ((rv = nni_zephyr_pollq_add_eventfd(pq)) != 0) {
		(void) close(pq->epfd);
		nni_mtx_fini(&pq->mtx);
		return (rv);
	}
	if ((rv = nni_thr_init(&pq->thr, nni_zephyr_poll_thr, pq)) != 0) {
		(void) close(pq->epfd);
		(void) close(pq->evfd);
		nni_mtx_fini(&pq->mtx);
		return (rv);
...
int
nni_zephyr_pollq_sysinit(void)
{
	return (nni_zephyr_pollq_create(&nni_zephyr_global_pollq));
}

void
nni_zephyr_pollq_sysfini(void)
{
	nni_zephyr_pollq_destroy(&nni_zephyr_global_pollq);
}
