...
nni_tcp_dialer_fini(nni_tcp_dialer *d)
{
	...
	nni_zephyr_tcp_dialer_rele(d);
}

void
nni_zephyr_tcp_dialer_rele(nni_tcp_dialer *d)
{
...
static void
tcp_dialer_cb(nni_zephyr_pfd *pfd, unsigned ev, void *arg)
{
	nni_tcp_conn *  c = arg;
...
nni_tcp_dial(nni_tcp_dialer *d, const nni_sockaddr *sa, nni_aio *aio)
{
	...
	if (((sslen = nni_zephyr_nn2sockaddr(&ss, sa)) == 0) ||
	...
	if ((rv = nni_zephyr_tcp_alloc(&c, d)) != 0) {
		nni_aio_finish_error(aio, rv);
		nni_zephyr_tcp_dialer_rele(d);
		return;
	}
	...
	// poll fd to the list.
	if ((rv = nni_zephyr_pfd_init(&pfd, fd)) != 0) {
		(void) close(fd);
		goto error;
	}

	nni_zephyr_tcp_init(c, pfd);
	nni_zephyr_pfd_set_cb(pfd, tcp_dialer_cb, c);
	...
	if (connect(fd, (void *) &ss, sslen) != 0) {
		...
		if ((rv = nni_zephyr_pfd_arm(pfd, NNI_POLL_OUT)) != 0) {
			goto error;
		}
		c->dial_aio = aio;
...
tcp_dialer_get_locaddr(void *arg, void *buf, size_t *szp, nni_type t)
{
	...
	nni_mtx_lock(&d->mtx);
	if (nni_zephyr_sockaddr2nn(&sa, &d->src, d->srclen) != 0) {
...
tcp_dialer_set_locaddr(void *arg, const void *buf, size_t sz, nni_type t)
{
	...
	if ((len = nni_zephyr_nn2sockaddr(&ss, &sa)) == 0) {
