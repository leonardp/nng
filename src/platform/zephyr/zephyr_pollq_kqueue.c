...
// nni_zephyr_pollq is a work structure that manages state for the kqueue-based
...
struct nni_zephyr_pfd {
	...
	nni_zephyr_pollq *pq;   // associated pollq
	int              fd;   // file descriptor to poll
	void *           arg;  // user data
	nni_zephyr_pfd_cb cb;   // user callback on event
	bool             closed;
	unsigned         events;
...
void
nni_zephyr_pfd_close(nni_zephyr_pfd *pf)
{
	nni_zephyr_pollq *pq = pf->pq;
...
void
nni_zephyr_pfd_fini(nni_zephyr_pfd *pf)
{
	nni_zephyr_pollq *pq;

	pq = pf->pq;

	nni_zephyr_pfd_close(pf);
...
int
nni_zephyr_pfd_fd(nni_zephyr_pfd *pf)
{
	return (pf->fd);
}

void
nni_zephyr_pfd_set_cb(nni_zephyr_pfd *pf, nni_zephyr_pfd_cb cb, void *arg)
{
...
int
nni_zephyr_pfd_arm(nni_zephyr_pfd *pf, unsigned events)
{
	struct kevent    ev[2];
	int              nev   = 0;
