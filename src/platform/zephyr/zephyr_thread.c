...
nni_plat_thr_init(nni_plat_thr *thr, void (*fn)(void *), void *arg)
{
	...
	// ZEPHYR wants functions to return a void *, but we don't care.
...
nni_plat_init(int (*helper)(void))
{
	...
	if ((rv = nni_zephyr_pollq_sysinit()) != 0) {
		pthread_mutex_unlock(&nni_plat_init_lock);
		pthread_mutexattr_destroy(&nni_mxattr);
		pthread_condattr_destroy(&nni_cvattr);
		pthread_attr_destroy(&nni_thrattr);
	...
	if ((rv = nni_zephyr_resolv_sysinit()) != 0) {
		pthread_mutex_unlock(&nni_plat_init_lock);
		nni_zephyr_pollq_sysfini();
		pthread_mutexattr_destroy(&nni_mxattr);
		pthread_condattr_destroy(&nni_cvattr);
		pthread_attr_destroy(&nni_thrattr);
		return (rv);
...
nni_plat_ncpu(void)
{
	// ZEPHYR specifies sysconf exists, but not the value
