...
static void
ipc_cb(nni_zephyr_pfd *pfd, unsigned events, void *arg)
{
	ipc_conn *c = arg;
...
void
nni_zephyr_ipc_start(nni_ipc_conn *c)
{
	nni_zephyr_pfd_set_cb(c->pfd, ipc_cb, c);
}
...
ipc_reap(void *arg)
{
	...
	if (c->dialer != NULL) {
		nni_zephyr_ipc_dialer_rele(c->dialer);
	}
...
int
nni_zephyr_ipc_alloc(nni_ipc_conn **cp, nni_sockaddr *sa, nni_ipc_dialer *d)
{
	ipc_conn *c;
...
void
nni_zephyr_ipc_init(nni_ipc_conn *c, nni_zephyr_pfd *pfd)
{
	c->pfd = pfd;
}
