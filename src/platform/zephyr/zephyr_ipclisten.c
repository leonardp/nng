...
ipc_listener_doaccept(ipc_listener *l)
{
	...
	while ((aio = nni_list_first(&l->acceptq)) != NULL) {
		...
		if ((rv = nni_zephyr_ipc_alloc(&c, &l->sa, NULL)) != 0) {
			(void) close(newfd);
			nni_aio_list_remove(aio);
			nni_aio_finish_error(aio, rv);
			continue;
		...
		nni_zephyr_ipc_init(c, pfd);

		nni_aio_list_remove(aio);
		nni_zephyr_ipc_start(c);
...
static void
ipc_listener_cb(nni_zephyr_pfd *pfd, unsigned events, void *arg)
{
	ipc_listener *l = arg;
	NNI_ARG_UNUSED(pfd);
...
ipc_listener_listen(void *arg)
{
	...
	if ((len = nni_zephyr_nn2sockaddr(&ss, &l->sa)) < sizeof(sa_family_t)) {
		return (NNG_EADDRINVAL);
	}
	...
	if ((rv != 0) || ((rv = nni_zephyr_pfd_init(&pfd, fd)) != 0)) {
		nni_mtx_unlock(&l->mtx);
		(void) close(fd);
		if (path != NULL) {
			unlink(path);
	...
	nni_zephyr_pfd_set_cb(pfd, ipc_listener_cb, l);
