...
// ZEPHYR AIO using poll().  We use a single poll thread to perform
...
// nni_zephyr_pollq is a work structure used by the poller thread, that keeps
...
{
	nni_zephyr_pfd *  pfd;
	nni_zephyr_pollq *pq = &nni_zephyr_global_pollq;
...
nni_zephyr_pollq_create(nni_zephyr_pollq *pq)
{
	...
	NNI_LIST_INIT(&pq->pollq, nni_zephyr_pfd, node);
