...
tcp_dowrite(nni_tcp_conn *c)
{
	...
	if (c->closed || ((fd = nni_zephyr_pfd_fd(c->pfd)) < 0)) {
		return;
	}
...
tcp_fini(void *arg)
{
	...
	tcp_close(c);
	if (c->pfd != NULL) {
		nni_zephyr_pfd_fini(c->pfd);
	}
	nni_mtx_fini(&c->mtx);

	if (c->dialer != NULL) {
		nni_zephyr_tcp_dialer_rele(c->dialer);
	}
	NNI_FREE_STRUCT(c);
}
...
static void
tcp_cb(nni_zephyr_pfd *pfd, unsigned events, void *arg)
{
	nni_tcp_conn *c = arg;
...
tcp_send(void *arg, nni_aio *aio)
{
	...
	if (nni_list_first(&c->writeq) == aio) {
		...
		// complete us.
		if (nni_list_first(&c->writeq) == aio) {
			nni_zephyr_pfd_arm(c->pfd, POLLOUT);
		}
	}
	nni_mtx_unlock(&c->mtx);
}
...
tcp_recv(void *arg, nni_aio *aio)
{
	...
	if (nni_list_first(&c->readq) == aio) {
		...
		// complete us.
		if (nni_list_first(&c->readq) == aio) {
			nni_zephyr_pfd_arm(c->pfd, POLLIN);
		}
	}
	nni_mtx_unlock(&c->mtx);
}
...
tcp_get_peername(void *arg, void *buf, size_t *szp, nni_type t)
{
	...
	int                     fd  = nni_zephyr_pfd_fd(c->pfd);
	int                     rv;
	nng_sockaddr            sa;
	...
	if ((rv = nni_zephyr_sockaddr2nn(&sa, &ss, len)) == 0) {
...
int
nni_zephyr_tcp_alloc(nni_tcp_conn **cp, nni_tcp_dialer *d)
{
	nni_tcp_conn *c;
...
void
nni_zephyr_tcp_init(nni_tcp_conn *c, nni_zephyr_pfd *pfd)
{
	c->pfd = pfd;
}

void
nni_zephyr_tcp_start(nni_tcp_conn *c, int nodelay, int keepalive)
{
	// Configure the initial socket options.
	(void) setsockopt(nni_zephyr_pfd_fd(c->pfd), IPPROTO_TCP, TCP_NODELAY,
	    &nodelay, sizeof(int));
	(void) setsockopt(nni_zephyr_pfd_fd(c->pfd), SOL_SOCKET, SO_KEEPALIVE,
	    &keepalive, sizeof(int));

	nni_zephyr_pfd_set_cb(c->pfd, tcp_cb, c);
}
